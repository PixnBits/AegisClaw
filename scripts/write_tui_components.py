#!/usr/bin/env python3
"""Writes internal/tui/components.go — reusable TUI components: table, spinner, modal, diff viewer."""
import os
...
outdir = os.path.join(os.path.dirname(__file__), '..', 'internal', 'tui')
outdir = os.path.abspath(outdir)
os.makedirs(outdir, exist_ok=True)
outpath = os.path.join(outdir, 'components.go')
with open(outpath, 'w') as f:
    f.write(code)
print(f"components.go: {len(code)} bytes -> {outpath}")
