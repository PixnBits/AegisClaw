#!/usr/bin/env python3
"""Writes internal/builder/builder.go — dedicated builder sandbox runtime."""
import os
...
outpath = os.path.join(os.path.dirname(__file__), '..', 'builder', 'builder.go')
outpath = os.path.abspath(outpath)
os.makedirs(os.path.dirname(outpath), exist_ok=True)
with open(outpath, 'w') as f:
    f.write(code)
print(f"builder.go: {len(code)} bytes -> {outpath}")
