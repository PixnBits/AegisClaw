#!/usr/bin/env python3
"""Writes internal/vault/vault.go — Kernel-managed secret vault using age encryption."""
import os
...
outpath = os.path.join(os.path.dirname(__file__), '..', 'internal', 'vault', 'vault.go')
outpath = os.path.abspath(outpath)
os.makedirs(os.path.dirname(outpath), exist_ok=True)
with open(outpath, 'w') as f:
    f.write(code)
print(f"vault.go: {len(code)} bytes -> {outpath}")
