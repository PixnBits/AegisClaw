#!/usr/bin/env python3
"""Writes internal/git/manager_test.go — unit tests for git manager."""
import os
...
outpath = os.path.join(os.path.dirname(__file__), '..', 'internal', 'git', 'manager_test.go')
outpath = os.path.abspath(outpath)
with open(outpath, 'w') as f:
    f.write(code)
print(f"manager_test.go: {len(code)} bytes -> {outpath}")
