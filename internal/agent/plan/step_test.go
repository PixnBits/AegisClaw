// step_test.go — unit tests for the Plan step (Phase 1.4 coverage push).
//
// These tests exercise the real Plan implementation with controlled inputs,
// including the local skill index (7.3) and memory context from a real Memory VM.
//
// SPEC REFERENCES:
//   - agent-runtime.md §Responsibilities (Plan step uses local tool index + Memory context)
...
func TestPlan_UsesMemoryContextAndSkillIndex(t *testing.T) {
	...
	tc := &agent.TurnContext{
		Input:              "User wants to create a new monitoring tool.",
	...
	tc.Input = map[string]interface{}{
		...
		"memory": map[string]interface{}{
			"short_term": []string{"User has autonomy grant for research."},
			"long_term":  []interface{}{"User previously discussed secure tool usage."},
		},
	}
	...
	llm := func(ctx context.Context, prompt string) (string, error) {
		...
		return "Plan: 1. Use local web_research tool via Hub. 2. Prepare proposal if needed. Memory context and local tools were considered.", nil
	}
	...
	if result == nil || result.Phase != "plan" {
		t.Errorf("expected plan phase, got %+v", result)
	}
	...
	if !contains(capturedPrompt, "autonomy grant for research") {
