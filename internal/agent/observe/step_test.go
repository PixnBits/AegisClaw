// step_test.go — unit tests for the Observe step (Phase 1.4 hardening).
//
// These tests exercise the real Observe implementation with controlled inputs,
// including the local skill index (7.3) and memory context from a real Memory VM.
//
// SPEC REFERENCES:
//   - agent-runtime.md §Responsibilities (Observe step uses local tool index + Memory context)
...
func TestObserve_UsesMemoryContextAndSkillIndex(t *testing.T) {
	...
	llm := func(ctx context.Context, prompt string) (string, error) {
		...
		return "Observed: Intent to create Discord monitoring skill. Requires Court proposal. Memory context and local tools were considered.", nil
	}

	result, err := Run(context.Background(), tc, llm)
	if err != nil {
		t.Fatalf("Observe.Run failed: %v", err)
	}

	if result == nil || result.Phase != "observe" {
		t.Errorf("expected observe phase, got %+v", result)
	}
