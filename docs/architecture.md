...
> **Any component that processes untrusted data or performs non-trivial logic must run inside its own dedicated Firecracker microVM.**

This creates strong, enforceable security boundaries at every major system interface.

### Core Principles

- **Minimal Trusted Computing Base (TCB)**: Only a very small, auditable host daemon runs with host-level privileges. All other functionality lives inside microVMs.
- **Strict Mediation**: No microVM may communicate directly with another. All communication is mediated and ACL-enforced by AegisHub.
...
The system is composed of one small trusted host component and multiple isolated microVMs.
...
- **AegisHub** — The central privileged router. Enforces ACLs and routes all communication between microVMs.
...
- **Governance Court VMs** — Five isolated microVMs (one for each persona: Coder, Tester, CISO, Security Architect, User Advocate).
- **Agent Runtime VMs** — Where the actual agent reasoning occurs. These are stateless.
- **Builder VMs** — Ephemeral VMs spun up per skill proposal to implement and build new skills.

Each microVM has a single, narrowly defined responsibility and a hard security boundary.

## Communication & Mediation

No microVM may communicate directly with any other microVM. All communication is strictly mediated by AegisHub.

### Communication Rules

- Every microVM has exactly one vsock connection to AegisHub.
...
2. **Court Review (Proposal)**  
   **Owner:** Court Scribe VM (Clerk of the Court)  
   The Scribe distributes the raw proposal to the five Governance Court VMs, collects votes and feedback, and returns the consolidated result.
...
4. **Implementation**  
   **Owner:** Builder VM (one per proposal)  
   A dedicated Builder VM creates a persistent git repository for the skill, implements the code, and submits a Pull Request-style revision.

5. **Code Review**  
   **Owner:** Court Scribe VM  
   The Scribe distributes the code changes to the five Court VMs and returns consolidated feedback to the Builder VM.
...
- **AegisHub**: Privileged router. Can see all inter-VM traffic.
