...
**Note (Phase 1 – 04-unix-socket-hardening branch, May 2026)**: DB-05 and DB-06 are now **In Progress** under `feature/04-unix-socket-hardening`. Core `SO_PEERCRED` + rate/size enforcement + `UnixPeerAllow` scaffold already landed in `internal/api/server.go` + `peer_uid_linux.go` (post-03). Full closure (allow-list, root reject, audit, expanded tests) targeted for this branch. See updated [04-unix-socket-hardening.md](../implementation-plan/04-unix-socket-hardening.md#implementation-notes-phase-1--may-2026) for details.
...
| DB-05 | P1 | **SO_PEERCRED** allow-list: reject unexpected UID/GID with stable audit | Unit + Linux integration | **In Progress (04 branch)**: `internal/api/server_unix_policy_linux_test.go` (`TestServer_UnixPeerAllowRejectsForeignUID`); wired in daemon (`start.go` `UnixPeerAllow`). Full allow-list + root/PID reject + audit targeted next. |
| DB-06 | P1 | **Rate limit and max message size** on Unix IPC | Unit / integration | **In Progress (04 branch)**: `internal/api/server_unix_policy_linux_test.go` (413 / 429); defaults in `internal/api/server.go`. Full per-PID + back-pressure + schema validation next. |
