...
- Remaining Phase 4/5: portal channel UI affordances, full E2E defaults/migration, more PM (LLM plans, monitoring). Richer PM started: user.goal handler now uses getPMPrompt() (with workspace SOUL/AGENTS) for dynamic, custom-aware plans instead of pure static template. Added `aegis pm goal <text> [--channel id]` CLI for direct triggering of the full flow (ensure + goal send + inspect channel). Makes PM usable from CLI for E2E testing/verification. Committed.
