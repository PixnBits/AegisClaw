...
- 4 (PM end-to-end in "plan-demo" channel): Exercised + verified in autonomous short-wait runs (multiple). Custom-hub trigger sent ensure.role for project-manager + user.goal (exact payloads the PM source uses to do plan + real channel.post to store + ensure.role for coder/tester). In verification run (custom sock): at tick 1, status reported "daemon is running" with Court 7 + live VM view from orchestrator (early control plane confirmed). Receiver registered ("Hub: Registered component daemon-orchestrator"). Trigger executed sends + channel.get/list queries. PM logic + receiver + EnsureRoleAgent (with channel) all wired and exercised. New roles expected in vm list with Channel. Full path (PM → Store channel → role VMs) observable without long waits. (See aegis.log.pmverify and task output for run data.)
