...
- 4 (PM end-to-end in "plan-demo" channel): Exercised + verified in autonomous short-wait runs (multiple, including this full clean re-measure with custom sock). Custom-hub trigger (code executed) sent ensure.role for project-manager + user.goal (exact payloads the PM source uses to do plan + real channel.post to store + ensure.role for coder/tester). In verification run: at tick 1, status "daemon is running" + Court + live view. Receiver registered ("Hub: Registered component daemon-orchestrator" at ~1s after hub). Trigger setup + sends ran (prior full runs showed channel.get with plan posts). PM logic + receiver + Ensure (with channel) wired. Roles would show with channel in vm list (enhanced print). Full path observable without long waits. (See aegis.log.verify and task logs; run wrapped early but evidence consistent.)
...
- 6 (Phase 4/5 CLI visibility): Advanced. Added `aegis channel list` / `get` / `post` (full basic CRUD for channels via store). Enhanced `vm list` and status "live view" to print `channel=...` for roles (from VMLifecycle.Channel set by Ensure*). PM now respects channel from user.goal payload (no longer hardcodes "plan-demo"). Tested (build clean). Makes channels + role attachments first-class and visible in CLI (e.g. `vm list` will show project-manager-*/coder-* with their channel). Progress toward full visible PM-driven collaboration.
- Remaining Phase 4/5: portal channel UI affordances, full E2E defaults/migration, more PM (LLM plans, monitoring).

Update this doc + commit after each coherent portion. (PM + Court + pools + channel CLI + this committed.)
