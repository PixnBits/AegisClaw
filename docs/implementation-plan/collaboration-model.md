...
- 6 (Phase 4/5 CLI visibility): Started. Added basic `aegis channel list` / `channel get <id>` (uses store channel.* via hub; shows plans, role posts, membership). Wired to root, --json support, help updated. Tested (build clean; errors gracefully without daemon/hub as expected). Makes the collaboration channels (PM posts, role activity) visible from CLI. More (post, @mentions) in follow-ups.
- Remaining Phase 4/5: richer PM, portal channel UI affordances, full E2E defaults/migration.
