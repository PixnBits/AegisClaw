...
- Remaining Phase 4/5: portal channel UI affordances (in progress), full E2E defaults/migration, more PM. Portal: replaced chat page/panel with channels page in web-portal static (add/list/archive channels via /api/channels, add/remove participants; defaults PM on create). Host: added handleHostChannelsAPI intercept for /api/channels* (maps to store channel.* incl new archive/add/remove_member). Store: extended with archive, add/remove_member (robust id/channel_id), create defaults project-manager member. UI: vanilla forms/lists for channels + members management. `aegis pm goal` + channel CLI also updated for consistency. Build clean. Committed. This fulfills "portal channel UI affordances". Next: polish, E2E defaults (auto main channel), or more PM.
