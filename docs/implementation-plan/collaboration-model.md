...
- Remaining Phase 4/5: portal channel UI affordances, full E2E defaults/migration, more PM (LLM plans, monitoring). Richer PM started: user.goal handler now uses getPMPrompt() (with workspace SOUL/AGENTS) for dynamic, custom-aware plans instead of pure static template. Committed.
