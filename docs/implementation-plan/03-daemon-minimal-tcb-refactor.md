...
|-------------|-------------|------|----------|---------|--------|
| Minimal privilege | Capability drop and bounding set invoked without panicking; stricter proofs where CAPs available | Unit / smoke | `cmd/aegisclaw/daemon_tcb_test.go` (`TestHardening_CapabilitiesDropCalled`, `TestHardening_CapabilityBoundingSetApplied`) | PR | **Implemented** (with env notes) — smoke in `daemon_tcb_test.go`; real post-drop capget + bounding verification + rlimit Getrlimit asserts in `daemon_hardening_linux_test.go`; non-privileged runs log/skip cleanly |
...
| Capability dropping | `dropCapabilities` / bounding set exercised | Unit | `daemon_tcb_test.go` | PR | **Implemented** (with env notes) — exercised + verified via new Linux tests (see above rows) |
| seccomp-bpf | Filter install hook runs | Unit | `TestHardening_SeccompFilterHook` | PR | **Implemented** (with env notes) — exercise hook in `daemon_tcb_test.go`; strict-mode kill proof via subprocess re-exec (blocked syscall → death) in `daemon_hardening_linux_test.go` |
