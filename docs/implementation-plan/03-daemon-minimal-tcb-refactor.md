...
| Lifecycle containment | On daemon exit, monitors stop cleanly; future: no orphaned VMs | Unit / INT | `cmd/aegisclaw/lifecycle_integration_test.go`; `cmd/aegisclaw/daemon_test.go` (monitor threshold); `cmd/aegisclaw/microvm_shutdown_test.go`; `cmd/aegisclaw/shutdown_all_sandboxes_test.go` (**DB-01** unit slice) | INT / PR | **Partial** — unit tests cover `terminateManagedHubAndStoreVMs` + `shutdownAllSandboxes`; full subprocess + Firecracker still [06](06-sandbox-lifecycle-containment.md) |
...
| Audit root signing | Merkle append, chain, tamper detection; sequential kernel actions | Unit | `internal/audit/merkle_test.go`; `internal/kernel/kernel_test.go` (`TestKernel_SignAndLogMerkle`, `TestKernel_MerkleAuditChainMultiEntries` **DB-02**) | PR | **Partial** — per-action signing verified; background “interval ticker” still **gap** if productized |
