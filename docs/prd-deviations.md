...
Updated: 2026-03-30 (D2-a resolved: full ReAct loop now runs inside agent VM via ToolProxy over vsock)
...
- **Update (2026-03-28)**: `DirectLauncher` (`internal/court/direct_launcher.go`) deleted — there is no opt-out from microVM isolation. `docs/architecture.md` §1 updated to make the no-opt-out rule explicit. D2-b resolved (daemon now forwards to agent VM). DC resolved (lazy agent VM startup via `ensureAgentVM`). D2-c (court DirectLauncher) resolved. D2-c-cli remains open.
- **Update (2026-03-30)**: D2-a resolved. The guest-agent now runs the full ReAct loop inside the microVM: calls Ollama via LLM proxy (vsock port 1025), parses tool-call blocks, executes tools via host-side ToolProxy (vsock port 1026), and loops until a final response. The daemon's `makeChatMessageHandler` and court round updater are now thin forwarders. Daemon-side `extractToolCallFromContent` fallback and nudge retry have been removed.
...
| D2 | PRD, CLI | The main agent should be a sandboxed component. | **Resolved** | The daemon's `makeChatMessageHandler` starts the agent microVM lazily (`ensureAgentVM`) and forwards each conversation as a single `chat.message`. The agent VM runs the full ReAct loop internally via LLM proxy (vsock 1025) and ToolProxy (vsock 1026). CLI `ExecuteTool` callbacks still run on the host (D2-c-cli). |
| D2-a | architecture.md §3, §7, §13.4 | Agent VM must run the full ReAct loop: parse tool-call blocks → execute tools via ToolProxy → append results to conversation → loop until clean response. | **Resolved** | `handleChatMessage` in `cmd/guest-agent/main.go` runs a full ReAct loop (up to 10 iterations). Tool calls are sent to the host-side ToolProxy (`cmd/aegisclaw/tool_proxy.go`) over vsock port 1026. The daemon's `makeChatMessageHandler` is a thin forwarder: one `SendToVM`, one final response. The daemon-side `extractToolCallFromContent` fallback has been removed. Future work: route tool calls through AegisHub for full ACL enforcement. |
